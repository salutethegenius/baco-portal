...
const fromEmail = process.env.SENDGRID_FROM_EMAIL || 'noreply@baco-bahamas.com';
const appUrl = process.env.APP_URL || 'http://localhost:5000';

export async function sendPasswordResetEmail(to: string, resetToken: string, userName?: string) {
  if (!process.env.SENDGRID_API_KEY) {
    console.warn('SendGrid not configured - password reset email not sent');
    return;
  }
  ...
  const msg = {
    to,
    from: fromEmail,
    subject: 'Reset Your BACO Portal Password',
    html: `
      <!DOCTYPE html>
      <html>
        <head>
...
export async function sendRegistrationConfirmationEmail(to: string, userName: string) {
  if (!process.env.SENDGRID_API_KEY) {
    console.warn('SendGrid not configured - registration confirmation email not sent');
    return;
  }
  ...
  const msg = {
    to,
    from: fromEmail,
    subject: 'Welcome to BACO Portal',
    html: `
      <!DOCTYPE html>
      <html>
        <head>
...
export async function sendEventRegistrationConfirmationEmail(
  ...
  if (!process.env.SENDGRID_API_KEY) {
    console.warn('SendGrid not configured - event registration confirmation email not sent');
    return;
  }
...
export async function sendMembershipRenewalReminderEmail(
  ...
  if (!process.env.SENDGRID_API_KEY) {
    console.warn('SendGrid not configured - membership renewal reminder email not sent');
    return;
  }
  ...
  const msg = {
    to,
    from: fromEmail,
    subject: 'BACO Membership Renewal Reminder',
    html: `
      <!DOCTYPE html>
      <html>
        <head>
...
Bahamas Association of Compliance Officers
  ...
  try {
    await sgMail.send(msg);
    console.log(`Membership renewal reminder email sent to ${to}`);
  } catch (error: any) {
    console.error('Error sending membership renewal reminder email:', error);
    throw new Error('Failed to send membership renewal reminder email');
  }
}
...
export async function sendDocumentApprovalEmail(
  ...
  if (!process.env.SENDGRID_API_KEY) {
    console.warn('SendGrid not configured - document approval email not sent');
    return;
  }
  ...
  const msg = {
    to,
    from: fromEmail,
    subject: 'Document Approved - BACO Portal',
    html: `
      <!DOCTYPE html>
      <html>
        <head>
...
Bahamas Association of Compliance Officers
  ...
  try {
    await sgMail.send(msg);
    console.log(`Document approval email sent to ${to}`);
  } catch (error: any) {
...
export async function sendDocumentRejectionEmail(
  ...
  if (!process.env.SENDGRID_API_KEY) {
    console.warn('SendGrid not configured - document rejection email not sent');
    return;
  }
  ...
  const msg = {
    to,
    from: fromEmail,
    subject: 'Document Review - Action Required - BACO Portal',
    html: `
      <!DOCTYPE html>
      <html>
        <head>
...
Bahamas Association of Compliance Officers
  ...
  try {
    await sgMail.send(msg);
    console.log(`Document rejection email sent to ${to}`);
  } catch (error: any) {
    console.error('Error sending document rejection email:', error);
    throw new Error('Failed to send document rejection email');
  }
}
